...
class SQL_Pandas_Parser():
                        ...
                        if k == "case":
                            ...
                            if type(cols) == str:
                                if '.' in cols:
                                    ...
                                    final_col.append(cols)
                                    print("table dict before final col:", self.tableColumnsDict)
                                    if self.tableNames[0] in self.tableColumnsDict.keys():
                                ...
                                for i in cols:
                                    if type(i) == str:
                                        if '.' in i:
                                            ...
                                            col_name = column_name[1]
                                            final_col.append(col_name)
                                            tableName = self.tableAlias[column_name[0]]
                                            if col_name[1] not in self.tableColumnsDict[tableName].keys():
                                                self.tableColumnsDict[tableName][col_name[1]] = ""
                                        else:
                                            final_col.append(i)
    ...
    #                 tableName = self.tableAlias[splits[0]]
    #                 columnName = splits[1]
    #                 if tableName in self.allColumns.keys():
    #                     self.allColumns[tableName].append(columnName)
    #                 else:
    #                     self.allColumns[tableName] = [columnName]
    #             else:
    #                 if self.tableNames[0] in self.allColumns.keys():
    #                     self.allColumns[self.tableNames[0]].append(val)
    #                 else:
    #                     self.allColumns[self.tableNames[0]] = [val]
    #         elif type(val) == dict:
    #             self.exploreDict(val)
    #         elif type(val) == list:
    #             for column in val:
...
if __name__ == "__main__":
    ...
    query = """create table newTable SELECT count(B.alpha) as alpha1,
        ...
        case when a.CO_RN_Goal =0 then true when a.CO_RN_Goal< b.alpha then true else false end as newCol,
        A.marsha as MARS,
    ...
    # query = """SELECT * ,marsha AS marsha2,FORMAT(stay_year, VARCHAR(7)) AS stay_year, marsha2 AS marsha,
    #         CASE WHEN co_rev_goal=' ' AND co_rev_goal=' ' OR co_rn_goal=0 OR co_rev_goal=0 THEN 0 ELSE co_rev_goal/co_rn_goal END AS co_rn_goal_adr,
    #         CASE WHEN def_otb=' ' OR def_rev=' ' OR def_otb=' ' OR def_rev=0 THEN 0 ELSE def_rev/def_otb END AS def_adr
    #         FROM Currents A JOIN Futures B ON A.marsha2 = B.marsha2 AND A.stay_year = B.stay_year"""

    query = """CREATE TABLE somethingelse AS  ( SELECT marsha, stay_year, coalesce( crossover_rms,0) AS CO_RN_Goal, 
    coalesce( ( crossover_rms*crossover_gadr),0) AS CO_Rev_Goal, def_rms AS Def_OTB, ( def_rms*def_gadr) AS Def_Rev, CID_Rms AS Target, Avg_rms AS Avg_Bkd FROM AW_TGT_BUS_DBO.OY_ANNUALCROSSOVER WHERE stay_year < &YEARNXT3 AND ASOF_YRMO=&CURYRPD ORDER BY marsha, stay_year);"""
    ...
    # query = """INSERT INTO TABLE AWPROD.STR_STARVIEW_COMP_STG SELECT * FROM work.starcomp;"""
    #
    # query = """CREATE TABLE srl_data AS SELECT b.*, coalesce( c.act_ct, 0) AS act_ct FROM data_2a AS b
    # LEFT JOIN data_3a AS c on b.prop_code=c.prop_code AND b.year=c.year AND b.month=c.month
    # ORDER BY b.prop_code, b.year, b.month;"""
    #
    # query = """CREATE TABLE data_2a AS SELECT distinct prop_code, year( stay_dt) AS year, month( stay_dt) AS month,
    # COUNT( user_capped) AS cap_ct FROM uat1_d GROUP BY prop_code, year, month;"""
    #
    # query = """CREATE TABLE PLU AS  ( SELECT a.*, b.ERS2_USER FROM AW_TGT_BUS_DBO.PROPERTY_MASTER A LEFT JOIN AW_TGT_PROXY_DBO.IT_OY_PROP_CD_WEEKLY B on a.marsha=b.prop_code WHERE openflag = 'Y' AND project_status='Open' AND interface='2' );"""
    #
    # query = """CREATE TABLE merge_CrossOver1 AS ( SELECT * ,marsha AS marsha2,FORMAT(stay_year, VARCHAR(7)) AS stay_year , marsha2 AS marsha , CASE WHEN co_rev_goal=' ' OR co_rev_goal=' ' OR co_rn_goal=0 OR co_rev_goal=0 THEN 0 ELSE co_rev_goal/co_rn_goal END AS co_rn_goal_adr, CASE WHEN def_otb=' ' OR def_rev=' ' OR def_otb=' ' OR def_rev=0 THEN 0 ELSE def_rev/def_otb END AS def_adr FROM Currents A JOIN Futures B ON A.marsha2 = B.marsha2 AND A.stay_year = B.stay_year);"""
    # query ="""CREATE TABLE sfodata.CrossOver_YTD AS ( SELECT  *  FROM merge_CrossOver1 );"""
    a = SQL_Pandas_Parser(query)
    # print(parse(a.sqlQuery)['select'])
    # a.identifyTables()
    # print(a.tableNames)
    # print(a.identifyColumns())
    # print(a.tableAlias)
    # for cols in a.identifyColumns():
    #     print(cols['base_col'])
    # print("table col dict:", a.tableColumnsDict)
    # print(a.tableColumnsDict)
    # print(a.tableColumnsDict)
    # print("query: ",a.sqlQuery)
    # l = a.flatten_json(parse(a.sqlQuery))
    # print("l is: ", l)
    # for b in l.keys():
    #     if "value" in b:
    #         print(l[b])
    ...
    # print(a.sqlQuery)
    # cases = a.identifyCaseStatements()
    # print(cases)
    # for case in cases:
    #     queries = a.buildCaseQuery(case)
    #     for query in a.buildCaseQuery(case):
    #         print(query)