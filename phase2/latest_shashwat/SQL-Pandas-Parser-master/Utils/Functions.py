...
def handleUDFs(dataStructure, columnList, columnAlias, tableNames, tableAliases, tableColumnsDict, udfList):
    ...
    if type(dataStructure) == dict:
        ...
        if (type(updatedDataStructure) == list and type(updatedDataStructure[0]) != dict) or type(updatedDataStructure) == str:
            ...
            udfList,updatedDataStructure= handleUDFs(updatedDataStructure, columnList, columnAlias, tableNames, tableAliases, tableColumnsDict,
                       udfList)
            print("(1)updated ds in lf :",updatedDataStructure)


        elif type(updatedDataStructure) == dict:

            key = list(dataStructure.keys())[0]
            print("(2)keys are",key)
            if updatedDataStructure in udfList:
                updatedDataStructure= {key:columnAlias}
                print("(4)dict : ",updatedDataStructure)

            udfList,updatedDataStructure= handleUDFs(updatedDataStructure, columnList, columnAlias, tableNames, tableAliases, tableColumnsDict, udfList)
            print("(5)updated ds in lf :",updatedDataStructure)
...
def udfIter(dataStructure, columnList, columnAlias, tableNames, tableAliases, tableColumnsDict, udfList, index):
    if index >= len(dataStructure):
        ...
        print("(6)ds:",dataStructure[index])
        udfList,ds = handleUDFs(dataStructure[index], columnList, columnAlias, tableNames, tableAliases, tableColumnsDict, udfList)
        print("(8) ds of index:",dataStructure[index])
        if type(dataStructure[index])==dict and type(list(dataStructure[index].values())[0])==dict:
            key=list(dataStructure[index].keys())[0]

        if ds in udfList:
            dataStructure[index] = columnAlias
        index = index + 1
        print("(7) ds in udfiter:",dataStructure)
